 ...
 * This is a tech debt.
 *
 * This task run migration in a localhost database because the flyway setup of in CircleCI
 * with postgres db was really hard.
...
object RunMigrations {
  ...
  private def getListOfFiles(dir: String):List[File] = {
    ...
    if (d.exists && d.isDirectory) {
      d.listFiles.filter(_.isFile).toList.sortBy(_.getName)
    } else {
      List[File]()
    }
  }
